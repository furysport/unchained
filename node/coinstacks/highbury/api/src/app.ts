...
import { middleware, ConnectionHandler, Registry, BlockHandler, TransactionHandler } from '@sudophunk/common-api'
import { getAddresses, NewBlock, Tx as BlockbookTx, WebsocketClient } from '@shapeshiftoss/blockbook'
import { utxo } from '@sudophunk/common-api'
...
app.get('/health', async (_, res) =>
  res.json({ status: 'up', network: 'highbury', connections: wsServer.clients.size })
)

const options = {
  customCss: '.swagger-ui .topbar { display: none }',
  customSiteTitle: 'ShapeShift Highbury API Docs',
